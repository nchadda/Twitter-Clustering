...
void Greedy::move(bool &moved){
  ...
  for(int k=0;k<Nnode;k++){
    ...
    if(bestM != oldM){
      ...
      mod_outFlow[oldM] -= node[flip]->outFlow;
      ...
      mod_outFlow[bestM] += node[flip]->outFlow;
...
void Greedy::initiate(void){
  ...
  for(int i=0;i<Nnode;i++){
    ...
    if(!node[i]->outLinks.empty()){
      int NoutLinks = node[i]->outLinks.size();
      for(int j=0;j < NoutLinks; j++){
        node[i]->outLinks[j].second = pr_size[i]*node[i]->outLinks[j].second/sum;
      }
...
void Greedy::tune(void){
  ...
  // !!!!!!Cannot calculate enter flow without using information from other subnetworks
  // enter_log_enter = 0.0;
  // exit_log_exit = 0.0;
  // size_log_size = 0.0;
  // enterFlow = 0.0;
...
void Greedy::determMove(vector<int> &moveTo){
  ...
  for(int i=0;i<Nnode;i++){
    ...
    if(newM != oldM){
      ...
      mod_outFlow[oldM] -= node[i]->outFlow;
      ...
      mod_outFlow[newM] += node[i]->outFlow;
